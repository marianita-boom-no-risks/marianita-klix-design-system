...
function generateReactComponent(comp) {
  ...
  const propsStr = comp.props
    ...
    .join(",\n");
  const classExpr = comp.cssClasses?.[0] || `mk-${comp.slug}`;
...
const server = new McpServer({
  name: "marianita-klix-design-system",
  version: "1.0.0",
});
...
server.tool(
  "list_components",
  "List all available components in the Marianita Klix Design System, optionally filtered by category. Categories: Form Controls, Data Display, Feedback, Navigation, Layout, Overlay, Content.",
  ...
  async ({ category }) => {
    ...
    let output = `# Marianita Klix Design System — ${filtered.length} Components\n\n`;
...
server.tool(
  ...
  async ({ name, format = "html" }) => {
    ...
    output += `\n\n## Code (${format})\n\n`;
    if (format === "html") {
      const base = comp.cssClasses?.[0] || `mk-${comp.slug}`;
...
server.tool(
  "get_tokens",
  "Get design tokens for the Marianita Klix Design System in CSS, SCSS, JSON, or Tailwind format.",
  {
    format: z
  ...
  async ({ format = "css", category }) => {
    ...
    return {
      content: [
        {
          type: "text",
          text: `# Marianita Klix Tokens — ${format.toUpperCase()}${category ? ` (${category})` : ""}\n\n\`\`\`${format === "tailwind" ? "js" : format}\n${output}\n\`\`\``,
        },
      ],
    };
  }
...
server.tool(
  "search",
  "Search across all components, tokens, and guidelines in the Marianita Klix Design System.",
  {
...
server.tool(
  "get_css_setup",
  "Get the complete CSS setup needed to use the Marianita Klix Design System in a project — tokens, base styles, and dark mode.",
  {},
  async () => {
    const css = `/* ============================================
   Marianita Klix Design System — CSS Setup
   ============================================ */
